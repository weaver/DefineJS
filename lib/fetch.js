...
Request.prototype.save = function(base, next) {
  ...
  function accept(resp) {
    base.tempFile({ suffix: suffix }, function(err, stream, file) {
      err ? next(err) : U.redirect(resp, stream, function(err) {
        next(err, file, resp);
      });
    });
  }
...
function request(method, uri, next) {
  ...
  var state = U.listen()
    ...
    .on(request, { response: onResponse, error: onError })
    .start(next);

  request.end();

  function onResponse(resp) {
    state.done(null, resp);
  }
...
function zcat(path, into, next) {
  U.redirect(Child.spawn('zcat', [path]).stdout, into, next);
}