...
Context.prototype.initSync = function(script) {
  this._bootstrap();
  script ? this._root._runScriptSync(script) : this._root.loadSync('');
  return this;
};
...
Package.prototype.initSync = function() {
  if (!this.isReady()) {
    ...
    this.configure(qname, U.loadJankyJSON(R.readSync(qname.uri())));
  }
  return this;
};
...
Package.prototype._loadSync = function(qname) {
  ...
  if (ext == '.js' || this.ctx.isRegistered(ext)) {
    ...
    var mod = this.modules[qname.lname] = new Module(qname.uri(), file),
        code = R.readSync(file);

    return loadModuleSync(this, qname, mod, code);
  }
  else if (ext == '.node') {
    var mod = this.modules[qname.lname] = new Module(qname.uri());
    return loadNativeSync(this, qname, mod);
  }
  else
    throw new Error('Unrecognized extension: "' + file + '".');
