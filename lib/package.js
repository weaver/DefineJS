...
Context.prototype._bootstrapSync = function() {
  if (!this.cacheRoot) {
  ...
  this._cache = new R.Cache(this.cacheRoot, { preinstall: R.compileNative });
...
Package.prototype.initSync = function() {
  if (!this.isReady()) {
    ...
    this.configure(qname.absolute(), U.loadJankyJSON(R.readSync(qname.uri())));
  }
  return this;
};
...
Package.prototype.resolveSync = function(name, relativeTo) {
  ...
  if (name in this.defined) {
    //console.log('resolve defined', name);
    return qname;
  }
  else if (qname.ns in this.nsmap) {
    //console.log('resolve nsmap', qname.ns, this.nsmap[qname.ns], qname.lname);
...
Package.prototype.loadSync = function(name, relativeTo) {
  ...
  //console.log('loadSync: resolved (%s, %s) -> %s', name, relativeTo, qname);
