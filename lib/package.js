...
Package.prototype.configure = function(qname, conf) {
  ...
  this._lib = qname.resolve('./lib/');
  this._nsmap = this._makeNSMap(conf.mappings || {});
  this._modules = {};
...
Package.prototype.resolveSync = function(name, relativeTo) {
  ...
  if (isRelative(name))
    return (relativeTo || this.qname).resolve(name);
  else if (isAbsolute(name))
    return name;

  var qname = QName.make(name),
      ref = this._nsmap[qname.ns];
...
Package.prototype.loadSync = function(name, relativeTo) {
  ...
  else if ((cached = this._modules[qname.lname]))
    return cached;
  else
...
Package.prototype._loadSync = function(qname) {
  Assert.equal(qname.ns, this._lib.ns, 'Namespace mismatch');
  ...
  if (ext == '.js' || this.ctx.isRegistered(ext)) {
    ...
    var mod = this._modules[qname.lname] = new Module(qname, file),
        code = R.readSync(file);

    return loadModuleSync(this, qname, mod, code);
  }
  else if (ext == '.node')
    return (this._modules[qname.lname] = loadNativeSync(qname));
  else
    throw new Error('Unrecognized extension: "' + file + '".');
};

Package.prototype._findModuleSync = function(qname) {
  var file = this.ctx._findModuleSync(this._lib.resolve(qname.lname));
  if (!file)
