...
function digestSync(path, method, none) {
  try {
    ...
    if (x.code == 'ENOENT' && none !== undefined)
      return none;
    throw x;
  }
};
...
function digest(path, method, none, next) {
  ...
  function done(err) {
    if (err && err.code == 'ENOENT' && none !== undefined)
...
function loadSync(path, method, opt) {
  ...
  try {
    ...
    if (x.code == 'ENOENT' && opt.none !== undefined)
      return opt.none;
    throw x;
  }
}
...
function load(path, method, opt, next) {
  ...
  Fs.readFile(path, opt.encoding || null, function(err, data) {
    if (err && err.code == 'ENOENT' && opt.none !== undefined)
      next(null, opt.none);
    else if (err)
      next(err);
    else
...
function pipe(input, output, next) {
  var state = listen()
    .on(input, { end: onEnd, error: onError })
